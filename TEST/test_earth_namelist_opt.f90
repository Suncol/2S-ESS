program test_earth_namelist_opt
...
!  START CODE $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
   ...
   write(*,*)
   write(*,*) 'Doing optimized 2S-ESS calculation with Earth namelist configuration'
   write(*,*)

!  Load Earth configuration from namelist
   write(*,*) 'Loading Earth configuration from namelist...'
   call load_planet_config_from_file('/home/suncong/work/2S-ESS/INPUT/earth_config.nml')
   planet_cfg = get_planet_config()
   
   write(*,*) 'Configuration loaded:'
   write(*,*) '  Planet: ', trim(planet_cfg%name)
   write(*,*) '  Radius: ', planet_cfg%radius_km, ' km'
   write(*,*) '  Has oceans: ', planet_cfg%has_oceans
...
!  Copy the earth radius, and height grid - USE PLANETARY CONFIGURATION

   nlayers = E_nlayers
   FO_nlayers = nlayers
   eradius = planet_cfg%radius_km    ! KEY CHANGE: Use namelist radius
   FO_eradius = eradius
   do n = 0, nlayers
...
!  Earth radius and heights - USE PLANETARY CONFIGURATION

   HEIGHT_GRID  = heights(0:nlayers)
   EARTH_RADIUS = planet_cfg%radius_km    ! KEY CHANGE: Use namelist radius
...
write(0,*) 'Planet radius used = ', planet_cfg%radius_km, ' km'


   ! output data into a file 
   open(1, file = 'test_earth_namelist_opt_output.dat')   
      write(1,*) '2S intensity = ', intensity_2S_Exact(1,1)
      write(1,*) 'ESS intensity = ', intensity_FO_Exact(1,1) 
      write(1,*) 'Planet = ', trim(planet_cfg%name)
      write(1,*) 'Radius = ', planet_cfg%radius_km
   close(1) 

   open(2, file = 'test_earth_namelist_opt_timings.dat')     
      ...
      write(2,*) 'Optical property calc time = ', Exacttimes(5)
      write(2,*) '2S calc time = ', Exacttimes(6)
      write(2,*) 'ESS calc time = ', Exacttimes(7)
...
end program test_earth_namelist_opt